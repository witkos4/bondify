...
export async function createTeamAs(user: TestUser, name: string, cleanup: CleanupRegistry): Promise<TeamFixture> {
  ...
  const { error: createTeamError } = await user.client.from("teams").insert({
...
export async function inviteToTeamAs(owner: TestUser, teamId: string, email: string): Promise<InviteFixture> {
  ...
  const { error } = await owner.client.from("team_invites").insert({
    accepted_at: null,
...
export async function acceptInviteAs(user: TestUser, inviteId: string): Promise<AcceptedInviteFixture> {
  ...
  const { data: invite, error: inviteReadError } = await user.client
    .from("team_invites")
  ...
  const { data: updatedInvite, error: inviteUpdateError } = await user.client
    .from("team_invites")
    .update({
...
export async function openRoundAs(user: TestUser, teamId: string, slug: string): Promise<RoundFixture> {
  ...
  const { error } = await user.client.from("game_rounds").insert({
...
export async function createEmojiSessionAs(
  ...
  const { error } = await user.client.from("emoji_check_in_sessions").insert({
    id: sessionId,
...
export async function prepareTwoTruthsVotingRound(input: {
  ...
  const { error: ownerEntryError } = await input.owner.client.from("two_truths_entries").insert({
  ...
  const { error: memberEntryError } = await input.member.client.from("two_truths_entries").insert({
  ...
  const { error: includeEntriesError } = await input.owner.client
  ...
  const { error: phaseError } = await input.owner.client
