...
| e2e                | Playwright/chromium                                  | `^1.61.0`                                                   | Browser specs under `tests/browser/*.spec.ts` are wired into CI after the Vitest suite; Playwright auto-boots `npm run dev:local` against the same local Supabase stack   |
...
- Keep CI on the repo's default branch (`main`) and pin third-party actions to
  immutable SHAs with the source tag in a trailing comment.
- Install dependencies with `npm ci`, then run `npx astro sync`, lint, and
  build before starting the heavier test stack.
- Start the local Supabase stack inside the job with `npx supabase start`.
  The CLI comes from the repo devDependency and applies migrations plus
  `supabase/seed.sql` on first boot.
- After the stack is healthy, capture credentials with
  `npx supabase status -o env`, then write them to `$GITHUB_ENV` under both
  the test-helper names and app runtime names:
  `BONDIFY_TEST_SUPABASE_URL` from `API_URL`,
...
- Install Chromium with `npx playwright install --with-deps chromium`, then
  run `npm run test:e2e`. The Playwright web server inherits the same
  `SUPABASE_URL` and `SUPABASE_KEY` values through the job environment.
